...
- **flow** — a reference to a named child sub-flow (see \`childFlowId\`); double-click in the UI to drill in. A sub-flow may finish in more than one state: each \`end\` node inside the child flow is a named **exit**, and the flow node's outgoing edges carry \`sourceHandle\` (the id of that \`end\` node) and \`data.exit\` (its name) to say which outcome the edge follows. Treat a flow node with several distinct \`data.exit\` values as a branching point, not an opaque box. Sub-flows nest: a child flow may itself contain flow nodes, to any depth. A sub-flow may also be entered at more than one point: each \`start\` node inside the child flow is a named **entry**, and edges arriving at the flow node carry \`targetHandle\` (the id of that \`start\` node) and \`data.entry\` (its name). Several distinct \`data.entry\` values mean work arrives into the sub-flow by genuinely different routes — worth noting when the routes are handled the same way once inside.
...
- **start** — pipeline entry point or sub-flow entry; no incoming edges in normal usage. Inside a child flow, each \`start\` node **is** one of that sub-flow's named entries — the mirror of \`end\` below: its \`name\` is how that route in is described, and it maps onto the parent's flow node via the \`targetHandle\`/\`data.entry\` on that node's incoming edges. A single \`start\` that fans out to several nodes means one way in and parallel work after it, not several ways in.
...
When reading edges from a decision node, use \`data.branch\` (or \`label\`) to understand which path is taken under which condition. When reading edges from a flow node, use \`data.exit\` the same way — it names the sub-flow outcome that leads down that path. Edges *into* a flow node carry \`data.entry\`, naming which way in they take.
